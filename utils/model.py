...
class Perceptron:
  def __init__(self, eta, epochs):
    ...
    print(f"initisl weights before training: {self.weights}")
  ...
  def fit(self, X, y):
    ...
    X_with_bais = np.c_[self.X, -np.ones((len(self.X), 1))] # Concatination
    ...
    for epoch in range(self.epochs):
      ...
      self.weighs = self.weights + self.eta * np.dot(X_with_bais.T, self.error) # backward propagation
